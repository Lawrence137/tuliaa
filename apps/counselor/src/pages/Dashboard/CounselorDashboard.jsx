...
// Placeholder for icons - in a real app, you would use an icon library like react-icons
const Icon = ({ name, className }) => {
    ...
    return (
        <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor">
            {icons[name] || <path d="M12 2.252a1 1 0 011 1v17.496a1 1 0 01-2 0V3.252a1 1 0 011-1z" />}
        </svg>
    );
};
    ...
    <div className="min-h-screen flex bg-gray-100 font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 w-64 bg-gradient-to-br from-indigo-700 to-purple-900 text-white flex flex-col transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0 transition-transform duration-300 ease-in-out z-30`}>
        ...
        <nav className="flex-1 p-4">
          <ul>
            <li className="mb-2">
              <Link to="/dashboard" className="flex items-center p-3 rounded-lg bg-purple-800 bg-opacity-50 text-white">
                <Icon name="dashboard" className="w-6 h-6 mr-3" />
                Dashboard
              </Link>
            </li>
            <li className="mb-2">
              <Link to="/calendar" className="flex items-center p-3 rounded-lg hover:bg-purple-800 hover:bg-opacity-50 transition-colors">
                <Icon name="calendar" className="w-6 h-6 mr-3" />
                Calendar
              </Link>
            </li>
            <li className="mb-2">
              <Link to="/sessions" className="flex items-center p-3 rounded-lg hover:bg-purple-800 hover:bg-opacity-50 transition-colors">
                <Icon name="video" className="w-6 h-6 mr-3" />
                Sessions
              </Link>
            </li>
            <li className="mb-2">
              <Link to="/billing" className="flex items-center p-3 rounded-lg hover:bg-purple-800 hover:bg-opacity-50 transition-colors">
                <Icon name="billing" className="w-6 h-6 mr-3" />
                Billing
              </Link>
            </li>
            <li className="mb-2">
              <Link to="/notes" className="flex items-center p-3 rounded-lg hover:bg-purple-800 hover:bg-opacity-50 transition-colors">
        ...
        <div className="p-4 border-t border-purple-800">
          <Link to="/settings" className="flex items-center p-3 rounded-lg hover:bg-purple-800 hover:bg-opacity-50 transition-colors">
            <Icon name="settings" className="w-6 h-6 mr-3" />
            Settings
          </Link>
          <Link to="/" className="flex items-center p-3 rounded-lg hover:bg-purple-800 hover:bg-opacity-50 transition-colors mt-2">
      ...
      <main className="flex-1 p-6 lg:p-10">
        ...
        <header className="flex justify-between items-center mb-8">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-2xl lg:text-3xl font-bold text-gray-800">Welcome, {user.name}!</h2>
              <p className="text-gray-600 hidden sm:block">Here's what's happening today.</p>
            </div>
          </div>
          <div className="flex items-center">
